...
plotting options:
    ...
    -min_genes_req   Choose minimum number of genes containing each GO term. Default=10.
...
if (defined $plot){
	...
	chomp $results;
	my $in_here="$binPath\/MakeHist_fromChartGO-11.pl";
	if ($in_here=~ m/ /){
	    $in_here=~ s/ /\\ /g;
	}
	if ($in_here=~ m/\(/){
	...
	#print "HERE: $in_here\n";
	print "$in_here $results $pval_cutoff $num_cutoff $plot_open $meth $sort_enrich $min_genes $max_genes $min_sig\n";
	`$in_here $results $pval_cutoff $num_cutoff $plot_open $meth $sort_enrich $min_genes $max_genes $min_sig`;
    }
}
	...
	#print "HERE: $in_here\n";
	print "$in_here $results $pval_cutoff $num_cutoff $plot_open $meth $sort_enrich $min_genes $max_genes $min_sig\n";
	`perl $in_here $results $pval_cutoff $num_cutoff $plot_open $meth $sort_enrich $min_genes $max_genes $min_sig`;
    }
}
